/**
 * Prima Veritas Dataset Materializer — LA Court Missing Pieces
 * NON-KERNEL CODE
 *
 * Responsibility
 * ---------------
 * Read the modified input.json (with deterministic removals),
 ...
 * Determinism Notes
 * ------------------
 * - Removals already applied by mutate_missing.mjs
 * - No randomness here
...
// 1. READ STRUCTURED INPUT (after removals)
...
const normalized = raw.map((record, index) =>
  normalizeStructured(record, {
    ingest_id: "la_court_missing_pieces",
...
console.log("LA COURT MISSING PIECES MATERIALIZED");
